...
/*!50001 VIEW `customer_orders_view` AS select `c`.`customer_name` AS `customer_name`,`o`.`order_id` AS `order_id`,`o`.`order_date` AS `order_date` from (`customers` `c` join `orders` `o` on((`c`.`customer_id` = `o`.`customer_id`))) */;
